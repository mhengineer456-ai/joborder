...
const SOURCES = {
  ...
  // 🔴 Index sheet (Lot + Shades + Saved At)
  index: {
    SHEET_ID: '1Hj3JeJEKB43aYYWv8gk2UhdU6BWuEQfCg5pBlTdBMNA', // <-- replace with your Index sheet id
    TAB_NAME: 'Index',
    RANGE: 'A1:Z',
    lotKey: 'Lot Number',
    shadesKey: 'Shades',
...
const findColumnKey = (rowObj, desiredKeyCandidates = []) => {
  ...
  const normMap = new Map(keys.map((k) => [k.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim(), k]));
  ...
  for (const k of keys) if (/\blot\b/.test(k.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim())) return k;
  return null;
};
...
const parseCuttingSheet = (values = []) => {
  ...
  for (let r = 0; r < values.length; r++) {
    ...
    const colorCol = lowerRow.findIndex(
      (s) => s === 'color' || s === 'colour' || s === 'shade' || s === 'shade color' || s === 'shade colour'
    );
...
const buildGeneratedMapsFromJobOrder = (rows, lotKeyName) => {
  ...
  const headerByNorm = new Map(keys.map((k) => [k.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim(), k]));

  const challanTotalKey =
    headerByNorm.get('challan total qty') || headerByNorm.get('challan total quantity') || 'Challan Total Qty';
...
/* ===========================
   Index → Lot+Shade → SavedAt map
   =========================== */
const buildSavedAtByLotShade = (rows, { lotKey, shadesKey, savedAtKey }) => {
  const map = {};
  for (const r of rows) {
    const lot = normalizeLot(r[lotKey]);
    if (!lot) continue;

    const rawShades = String(r[shadesKey] ?? '').trim();
    if (!rawShades) continue;

    const savedAtStr = r[savedAtKey];
    const savedAtDate = savedAtStr ? new Date(savedAtStr) : null;
    if (!savedAtDate || isNaN(savedAtDate.getTime())) continue;

    const shades = rawShades.split(',').map((s) => normalizeShade(s));
    if (!map[lot]) map[lot] = {};
    for (const sh of shades) {
      if (!sh) continue;
      const prev = map[lot][sh];
      // keep the latest timestamp if duplicates
      if (!prev || savedAtDate > prev) map[lot][sh] = savedAtDate;
    }
  }
  return map; // { [lot]: { [shade]: Date } }
};
...
const PrintingPendingChallans = () => {
  ...
  const [savedAtByLotShade, setSavedAtByLotShade] = useState({}); // ← Index map
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const embKey = findColumnKey(sec[0], ['Print', 'Printing', 'Design', 'Design Name']);
      if (!secKey) throw new Error(`Could not find JobOrder lot column (${SOURCES.second.lotKey}).`);

      const { shadeByLot, completeByLot } = buildGeneratedMapsFromJobOrder(sec, secKey);
      ...
      // Build SavedAt map from Index
      if (SOURCES.index.SHEET_ID !== 'PUT_INDEX_SHEET_ID_HERE' && idx.length) {
        const lotKey = findColumnKey(idx[0], [SOURCES.index.lotKey]) || SOURCES.index.lotKey;
        const shadesKey = findColumnKey(idx[0], [SOURCES.index.shadesKey]) || SOURCES.index.shadesKey;
        const savedAtKey = findColumnKey(idx[0], [SOURCES.index.savedAtKey]) || SOURCES.index.savedAtKey;
        const map = buildSavedAtByLotShade(idx, { lotKey, shadesKey, savedAtKey });
        setSavedAtByLotShade(map);
      } else {
        setSavedAtByLotShade({});
      }
  ...
  // Build pending challans (base dataset) + compute lot-level age (max hours among its pending shades)
  const pendingChallans = useMemo(() => {
    ...
    matchedRows.forEach((row) => {
      ...
      if (remainingItems.length > 0 && !isLotMarkedComplete(lot)) {
        const party = String(row['Party Name'] ?? row['Party'] ?? row['Buyer'] ?? row['Customer'] ?? '').trim();
        const style = String(row['Style'] ?? row['Style No'] ?? row['Style Name'] ?? '').trim();
        const emb = String(row['Print'] ?? row['Printing'] ?? row['Design'] ?? row['Design Name'] ?? '').trim();

        // Shade-level ages (from Index)
        const shadeAges = remainingItems.map((it) => {
          const saved = savedAtByLotShade?.[lot]?.[normalizeShade(it.shade)] || null;
          const hrs = saved ? hoursDiff(saved) : null;
          return { shade: it.shade, qty: it.qty, savedAt: saved, ageHours: hrs };
        });

        // Lot-level pending age: take max hours across its pending shades (so if any shade overdue, lot shows overdue)
        const lotAgeHours = shadeAges.reduce((max, s) => (s.ageHours != null ? Math.max(max, s.ageHours) : max), 0);
        const anyAgeKnown = shadeAges.some((s) => s.ageHours != null);
        ...
        pending.push({
          ...
          style,
          emb,
          pendingItems: shadeAges, // includes ageHours now
          totalPending: remainingItems.reduce((sum, item) => sum + item.qty, 0),
          lotAgeHours: anyAgeKnown ? lotAgeHours : null,
          rowData: row,
        });
      }
    });
  ...
  const partyOptions = useMemo(() => uniqueSorted(pendingChallans.map((d) => d.party)), [pendingChallans]);
  const styleOptions = useMemo(() => uniqueSorted(pendingChallans.map((d) => d.style)), [pendingChallans]);
  const embOptions = useMemo(() => uniqueSorted(pendingChallans.map((d) => d.emb)), [pendingChallans]);
  ...
  const filteredData = useMemo(() => {
    ...
    let data = pendingChallans.filter((d) => {
      ...
      return matchesSearch && matchesParty && matchesStyle && matchesEmb && minOk && maxOk;
    });

    if (sortBy === 'pendingAsc') data = data.slice().sort((a, b) => a.totalPending - b.totalPending);
    else if (sortBy === 'pendingDesc') data = data.slice().sort((a, b) => b.totalPending - a.totalPending);
    else data = data.slice().sort((a, b) => a.lot.localeCompare(b.lot));

    return data;
  }, [pendingChallans, search, partyFilter, styleFilter, embFilter, minPending, maxPending, sortBy]);
  ...
  const exportToCSV = (rows, filename = 'pending_challans_grouped_readable.csv') => {
    if (!rows?.length) return;

    const headers = ['Lot', 'Party', 'Style', 'Embroidery', 'Shade', 'Qty', 'Total Pending (Lot)'];
    const esc = (v) => {
    ...
    rows.forEach((d, idx) => {
      ...
      if (idx > 0) csvLines.push(''); // spacer

      const banner = `▶ LOT ${lot} — Party: ${party || '-'} • Style: ${style || '-'} • Emb: ${emb || '-'} • Total: ${total}`;
      csvLines.push([esc(banner), '', '', '', '', '', ''].join(','));
      ...
      const first = items[0];
      csvLines.push([esc(lot), esc(party), esc(style), esc(emb), esc(first.shade ?? ''), esc(first.qty ?? 0), esc(total)].join(','));
  ...
  const exportToXLSXStyled = async (rows) => {
    if (!rows?.length) return;

    let XLSX;
    let supportsStyles = false;
    try {
      XLSX = (await import('xlsx-js-style')).default || (await import('xlsx-js-style'));
      supportsStyles = true;
    } catch {
      try {
        ...
        exportToCSV(rows, 'pending_challans_grouped_readable.csv');
        return;
      }
    }

    const data = [];
    const header = ['Lot', 'Party', 'Style', 'Embroidery', 'Shade', 'Qty', 'Total Pending (Lot)'];
    data.push(header);
    ...
    rows.forEach((d, idx) => {
      ...
      if (idx > 0) data.push(['', '', '', '', '', '', '']); // spacer
      const banner = `▶ LOT ${lot} — Party: ${party || '-'} • Style: ${style || '-'} • Emb: ${emb || '-'} • Total: ${total}`;
      data.push([banner, '', '', '', '', '', '']);

      if (items.length === 0) {
        data.push([lot, party, style, emb, '', '', total]);
      } else {
        const first = items[0];
        data.push([lot, party, style, emb, first.shade ?? '', first.qty ?? 0, total]);
        for (let i = 1; i < items.length; i++) {
          const it = items[i];
          data.push(['', '', '', '', it.shade ?? '', it.qty ?? 0, '']);
        }
      }
    });
    ...
    ws['!cols'] = [{ wch: 36 }, { wch: 20 }, { wch: 16 }, { wch: 24 }, { wch: 20 }, { wch: 10 }, { wch: 18 }];
    ...
    if (supportsStyles) {
      ...
      for (let R = range.s.r; R <= range.e.r; ++R) {
        ...
        const aVal = cellA?.v || '';
        let rowType = 'normal';
        if (R === 0) rowType = 'header';
        else if (aVal === '' && (ws[XLSX.utils.encode_cell({ r: R, c: 1 })]?.v || '') === '') rowType = 'spacer';
        else if (typeof aVal === 'string' && aVal.startsWith('▶ LOT')) rowType = 'banner';

        for (let C = range.s.c; C <= range.e.c; ++C) {
          const addr = XLSX.utils.encode_cell({ r: R, c: C });
          ws[addr] = ws[addr] || { t: 's', v: '' };
          const baseBorder = { top: { style: 'thin', color: { rgb: 'E5E7EB' } }, bottom: { style: 'thin', color: { rgb: 'E5E7EB' } } };
    ...
    XLSX.writeFile(wb, 'pending_challans_styled.xlsx');
  };
  ...
  const exportToPDF = async (rows) => {
    ...
    try {
      ...
      for (const d of rows) {
        ...
        const body = (d.pendingItems || []).map((it) => [it.shade, (it.qty ?? 0).toLocaleString('en-IN')]);
    ...
    <div className="mh-ec">
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        table.minw{width:100%;min-width:1200px;border-collapse:separate;border-spacing:0;background:var(--panel-2)}
        ...
        .shade-meta{font-size:11px;opacity:.8}
        ...
        <div className="row">
          <div>
            <div className="title-row">
              ...
              <h1>Pending Printing Challans</h1>
            </div>
        ...
        <div className="toolbar">
          <div className="filters">
            <input
              ...
              placeholder="Search (Lot / Party / Style / Printing / Shade)"
              value={search}
            ...
            <select className="select" value={embFilter} onChange={(e) => setEmbFilter(e.target.value)}>
              <option value="">Printing (All)</option>
              {embOptions.map((e1) => (
          ...
          <div className="actions">
            ...
            <button className="btn" onClick={() => exportToXLSXStyled(filteredData)}>Export Excel</button>
                ...
                {filteredData.map((challan, i) => {
                  ...
                  return (
                    <tr key={challan.lot + '_' + i}>
                      ...
                      <td data-label="Printing">{challan.emb}</td>
                            ...
                            {challan.pendingItems.map((item, idx) => {
                              ...
                              return (
                                <div key={idx} className="shade-item">
                                  <div>
                                    <div>{item.shade}</div>
                                    <div className="shade-meta">Qty: {numberFormatIN(item.qty)}</div>
                                  </div>
                                  <div className={`shade-age ${shOverdue ? 'overdue' : ''}`}>
...
export default PrintingPendingChallans;
