...
const EmbroideryChallan = () => {
  ...
  const stats = useMemo(() => {
    ...
    let pendingEmbCount = 0;
    let completedEmbCount = 0;
    ...
    matchedRows.forEach((row) => {
      ...
      const emb = getembroiderystatus(row);
      if (emb === 'Complete Emb') {
        completedEmbCount++;
      } else if (emb === 'Pending') {
        pendingEmbCount++;
  ...
  const applyFilters = useMemo(
    () =>
      debounce((q, matF, embF, rows, cols) => {
        let next = rows;
        ...
        // Dropdown Embroidery filter
        if (embF !== 'ALL') {
          next = next.filter((row) => {
            const e = getembroiderystatus(row);
            if (embF === 'COMPLETED') return e === 'Complete Emb';
            if (embF === 'PENDING') return e === 'Pending';
            return true;
          });
        }
  ...
  useEffect(() => {
    applyFilters(query, matFilter, embFilter, matchedRows, visibleCols.length ? visibleCols : columns);
  }, [query, matFilter, embFilter, matchedRows, columns, visibleCols, applyFilters]);
  ...
  const buildA3PortraitChallanPDF = async (row, challanNo, challanDate, pdfDisplayItems, completeLotFlag, displayChallanNo, isDuplicate = false) => {
    ...
    const drawPageHeaderAndBorder = (doc, isDupe) => {
      ...
      const titleText = isDupe ? "EMB CHALLAN (DUPLICATE COPY)" : "EMBROIDERY CHALLAN";
    ...
    const drawTopHeaderOnly = (doc, isDupe) => {
      ...
      setFont(doc, "bold", F.h1, C.primary);
      const titleText = isDupe ? "EMBROIDERY CHALLAN (DUPLICATE COPY)" : "EMBROIDERY CHALLAN";
      doc.text(titleText, PAGE.w / 2, 62, { align: "center" });
  ...
  const handleCreateChallan = async (row, extras = {}) => {
    ...
    catch { challanNo = await reserveChallanNo('CH-EMB-', 0); }
      ...
      <div className="shell">
        ...
        <div className="hero">
          ...
          <div className="stats-container">
            ...
            <div className="stat-card">
              <div className="stat-lbl">Embroidery Status</div>
              <div className="stat-val">
                <span style={{ color: 'var(--warn)' }}>{stats.pendingEmbCount}</span> <span style={{ color: 'var(--muted)', fontSize: 13 }}>/</span> <span style={{ color: 'var(--ok)' }}>{stats.completedEmbCount}</span>
              </div>
          ...
          <div className="toolbar rel">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <select
                ...
                title="Filter by Embroidery Status"
              >
                <option value="ALL">Embroidery: All</option>
                <option value="COMPLETED">Embroidery: Completed</option>
                <option value="PENDING">Embroidery: Pending</option>
              </select>
            </div>
                      ...
                      return (
                        <tr key={i}>
                          {renderCols.map((c) => {
                            ...
                            return (
                              <td key={c} data-label={c} style={isLotCol ? { fontWeight: '900', fontSize: '15px', color: 'red' } : undefined}>
                                {row[c]}
                              </td>
                            );
                          })}
                          ...
                          <td data-label="Embroidery Status">
                            {embstatus ? (
                              <span className={`status-tag status-${embstatus === 'Complete Emb' ? 'success' : 'warning'}`} title={embstatus}>
                                <span className="status-dot"></span>
