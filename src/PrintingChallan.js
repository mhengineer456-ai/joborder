...
const postJSON = async (url, data) => {
  try {
    const backendEndpoint = `${BACKEND_API_URL}/challan/action`;
    ...
    console.warn('⚠️ Express Backend offline, falling back to Apps Script:', err.message);
  }

  const res = await fetch(url || APPSCRIPT_WEB_APP_URL, {
    method: 'POST',
...
const SOURCES = {
  ...
  cuttingScans: {
    SHEET_ID: '1UU9P1IOjuUYpm3Ojx06V5L-r6K2QRb4mpFQmlGblgDA',
    TAB_NAME: 'CuttingScanss',
    RANGE: 'A1:C',
  },
};
...
const getembroiderystatus = (row) => {
  ...
  return anyCompleted ? 'Pending' : 'Complete Printing';
};
const getChallanCountForLot = (lotNumber, challanHistory) => {
  const baseNumber = 'CH-PRINT-'; // Your base challan number prefix
  let count = 0;
...
const reserveChallanNo = async (series = 'CH-PRINT-', zeroPad = 0) => {
...
const setChallanEmbStatus = async ({ lotNumber, challanNumber, historyIndex, embCompleted }) => {
  const payload = {
    ...
    lotNumber,
    challanNumber,
    historyIndex,      // exact index we computed in UI
...
const COLS_KEY = 'mh-pc-visible-cols-v2';
const BORDERS_KEY = 'mh-pc-borders-v2';
...
const pickDefaultVisibleColumns = (allCols = []) => {
  const groups = [
    ...
    { id: 'lot', pats: ['lot number', 'lot no', 'lot'] },
    { id: 'fabric', pats: ['fabric', 'fabric type'] },
    { id: 'brand', pats: ['brand', 'brand name'] },
    { id: 'garment', pats: ['garment type', 'garment', 'gmt', 'gmt type', 'item name'] },
    { id: 'style', pats: ['style', 'style no', 'style name'] },
  ];

  const res = [];
  for (const group of groups) {
    const found = allCols.find((col) => {
      const n = normalizeHeader(col);
      return group.pats.some((p) => n.includes(p));
    });
    if (found) {
      res.push(found);
    }
  ...
  if (!res.length) return allCols.slice(0, Math.min(8, allCols.length));
  return res;
};
...
const parseCuttingSheet = (values = []) => {
  ...
  for (let r = 0; r < values.length; r++) {
    ...
    const tableCol = lowerRow.findIndex((s) =>
      s === 'cutting table' || s === 'table' || s === 'table no' || s === 'table no.' || s === 'table #' || s === 'tbl'
    );
    ...
    if (colorCol !== -1 && totalCol !== -1 && currentLot) {
      ...
      while (k < values.length) {
        ...
        const tableNoVal = tableCol !== -1 ? norm(prow[tableCol]) : '';
        if (shade) {
          const qty = Number(String(qtyRaw).replace(/,/g, '')) || 0;
          const tableNo = tableNoVal || String(rowIdx);
          if (!itemsByLot[currentLot]) itemsByLot[currentLot] = [];
          itemsByLot[currentLot].push({ shade, qty, tableNo });
          rowIdx++;
        }
        k++;
      }
...
const PrintingChallan = () => {
  ...
  const [cuttingDates, setCuttingDates] = useState({});                    // { [lot]: string (timestamp) }
  ...
  const urls = useMemo(
    () => ({
      ...
      cuttingScans: buildSheetsUrl(SOURCES.cuttingScans.SHEET_ID, SOURCES.cuttingScans.TAB_NAME, SOURCES.cuttingScans.RANGE, API_KEY),
    }),
    []
  );
  ...
  const getPositiveQtyItemsForLot = (rowOrLot) => {
    ...
    return items
      .map((it) => ({
        ...
        qty: Number(it.qty) || 0,
        tableNo: String(it.tableNo || '').trim(),
      }))
      .filter((it) => it.qty > 0);
  };
  ...
  const getRemainingItemsForLot = (rowOrLot) => {
    ...
    const cuttingMap = {};
    for (const { shade, qty, tableNo } of cutting) {
      const key = normalizeShade(shade);
      if (!cuttingMap[key]) {
        cuttingMap[key] = { qty: 0, tableNo: tableNo || '' };
      }
      cuttingMap[key].qty += qty;
      if (tableNo && !cuttingMap[key].tableNo) {
        cuttingMap[key].tableNo = tableNo;
      }
    }

    const remaining = [];
    for (const key of Object.keys(cuttingMap)) {
      const cutQty = cuttingMap[key].qty;
      ...
      if (rem > 0) {
        remaining.push({
          shade: key,
          qty: rem,
          tableNo: cuttingMap[key].tableNo || '',
        });
      }
    }
  ...
  const fetchBoth = async () => {
    try {
      ...
      const [resCut, resSec, resScans] = await Promise.all([
        fetch(urls.cutting).then((r) => r.json()),
        fetch(urls.second).then((r) => r.json()),
        fetch(urls.cuttingScans).then((r) => r.json()).catch(() => ({ values: [] })),
      ]);
      ...
      const scansVals = resScans?.values || [];

      const scanMap = {};
      if (scansVals.length > 1) {
        const headers = scansVals[0];
        const lotIdx = headers.findIndex((h) => String(h).trim().toLowerCase() === 'lot number');
        const timeIdx = headers.findIndex((h) => String(h).trim().toLowerCase() === 'timestamp');
        if (lotIdx !== -1 && timeIdx !== -1) {
          for (let i = 1; i < scansVals.length; i++) {
            const row = scansVals[i];
            const lot = normalizeLot(row[lotIdx]);
      ...
      const embKey = findColumnKey(sec[0], ['Printing', 'Print', 'Design', 'Design Name']);
  ...
  // ===== stats memoization for dashboard metrics =====
  const stats = useMemo(() => {
    let totalPendingQty = 0;
    let pendingPrintCount = 0;
    let completedPrintCount = 0;
    let pendingMatCount = 0;
    let receivedMatCount = 0;

    matchedRows.forEach((row) => {
      totalPendingQty += getLotPendingQtyNumber(row) || 0;

      const pStatus = getembroiderystatus(row);
      if (pStatus === 'Complete Printing') {
        completedPrintCount++;
      } else if (pStatus === 'Pending') {
        pendingPrintCount++;
      }

      const mat = getMaterialStatus(row);
      if (mat === 'Material Received') {
        receivedMatCount++;
      } else if (mat === 'Pending') {
        pendingMatCount++;
      }
  ...
  const applyFilters = useMemo(
    () =>
      debounce((q, matF, printF, rows, cols) => {
        let next = rows;
        ...
        if (matF !== 'ALL') {
          next = next.filter((row) => {
            const m = getMaterialStatus(row);
            if (matF === 'RECEIVED') return m === 'Material Received';
            if (matF === 'PENDING') return m === 'Pending';
            return true;
          });
        }

        // Dropdown Printing filter
        if (printF !== 'ALL') {
          next = next.filter((row) => {
            const p = getembroiderystatus(row);
            if (printF === 'COMPLETED') return p === 'Complete Printing';
            if (printF === 'PENDING') return p === 'Pending';
            return true;
          });
        }
        ...
        next = next.filter((row) => {
          ...
          const printingStatus = getembroiderystatus(row);
          ...
          return !(
            printingStatus === 'Complete Printing' &&
  ...
  useEffect(() => {
    applyFilters(query, matFilter, printFilter, matchedRows, visibleCols.length ? visibleCols : columns);
  }, [query, matFilter, printFilter, matchedRows, columns, visibleCols, applyFilters]);
  ...
  const exportCSV = () => {
    ...
    const enhancedRows = displayRows.map(row => {
      ...
      rowData['Printing Status'] = embroideryStatus || '';
  ...
  const openChallanModal = (row) => {
    ...
    if (remaining.length > 0) {
      // Show only pending shades with their remaining quantities and table numbers
      setItems(remaining.map((it) => ({ shade: it.shade, qty: String(it.qty), tableNo: it.tableNo || '' })));
    } else {
      ...
      if (isLotMarkedComplete(lot)) {
        ...
        if (missingShades.length > 0) {
          setItems(missingShades.map((shade, idx) => ({ shade, qty: '', tableNo: String(idx + 1) })));
        } else {
          setItems([{ shade: '', qty: '', tableNo: '' }]);
        }
      }
  ...
  const buildA3PortraitChallanPDF = async (row, challanNo, challanDate, pdfDisplayItems, completeLotFlag, displayChallanNo, isDuplicate = false) => {
    const first = (keys, fb = '-') => {
      for (const k of keys) {
    ...
    const emb = first(['Printing', 'Print', 'Design', 'Design Name']);
    const embDet = first(['Printing Details']);
    ...
    const IMG_MAX = 900;
    const F = { h1: 34, h2: 24, h3: 20, body: 15, label: 13, meta: 12, small: 11 };
    const C = {
      primary: [0, 0, 0],
      accent: [0, 0, 0],
      dark: [0, 0, 0],
    ...
    const imageBase64 = typeof loadImageAsBase64ForPdf === "function" && (row?.["Image URL"] || '').trim()
      ? await (async () => {
        try { return await loadImageAsBase64ForPdf(row["Image URL"], { maxWidth: IMG_MAX, maxHeight: IMG_MAX }); }
        catch { return null; }
      })()
      : null;
    ...
    const pairs = [
      ...
      { label: "Printing", value: emb || '-' },
      { label: "Printing Details", value: embDet || '-' },
      { label: "Embroidery", value: first(['Emb', 'Embroidery', 'Design', 'Design Name']) || '-' },
      { label: "Embroidery Details", value: first(['Emb Details', 'Embroidery Details']) || '-' },
      ...
      { label: "Cutting Date", value: cuttingDateFromSheet },
    ];
    ...
    const drawPageHeaderAndBorder = (doc, isDupe) => {
      // Header & border
      ...
      setFont(doc, "bold", F.h1, C.primary);
      const titleText = isDupe ? "PRINT CHALLAN (DUPLICATE COPY)" : "PRINTING CHALLAN";
      doc.text(titleText, PAGE.w / 2, 62, { align: "center" });
      ...
      const yRemarksImg = boxY + boxH + GRID_ROW_GAP;

      // ------- Full Remarks & Image -------
      const colGap = GRID_COL_GAP;
      const colW2 = (boxW - colGap) / 2;
      const leftX = M;
      const rightX = M + colW2 + colGap;
      const yTop = yRemarksImg;
      ...
      if (imageBase64) {
        try {
          const p = doc.getImageProperties(imageBase64);
          ...
          doc.addImage(imageBase64, "JPEG", ix, iy, iw, ih);
        } catch (err) {
          setFont(doc, "normal", F.meta, C.grayDark);
      ...
      return Math.max(yTop + cardH, yTop + estRightH) + GRID_ROW_GAP;
    };

    const drawTopHeaderOnly = (doc, isDupe) => {
      doc.setFillColor(...C.white);
      doc.rect(0, 0, PAGE.w, 90, "F");
      doc.roundedRect(M, 70, PAGE.w - M * 2, PAGE.h - 160, 12, 12, "F");
      doc.setDrawColor(...C.border);
    ...
    // Draw on page 1
    const startTableY = drawPageHeaderAndBorder(doc, isDuplicate);

    // ------- Cutting Schedule table (ONLY ACTUAL DATA ROWS) -------
    const cuttingItemsForLot = getPositiveQtyItemsForLot(lot);
    const tableNoMap = {};
    for (const item of cuttingItemsForLot) {
      if (item.shade && item.tableNo) {
        tableNoMap[normalizeShade(item.shade)] = item.tableNo;
      }
    }

    const shadesArr = (pdfDisplayItems || []).map((it, idx) => {
      const normS = normalizeShade(it.shade);
      const tableVal = it.tableNo || tableNoMap[normS] || (idx + 1);
      return { tableNo: tableVal, shade: it.shade, qty: Number(it.qty) || 0 };
    });

    const tableData = shadesArr.map((r, idx) => [
      idx + 1,
      r.tableNo,
      r.shade,
      fmtNum(r.qty),
    ...
    autoTable(doc, {
      startY: startTableY,
      margin: { top: 396, left: M, right: M },
      theme: 'grid',
      head: [['Sr. No.', 'Table No.', 'Shade', 'Quantity', 'Date of Issue']],
      body: tableData,
      styles: {
        fontSize: 10,
        cellPadding: 6,
      ...
      columnStyles: {
        ...
        2: { cellWidth: (PAGE.w - M * 2 - 106) * 0.54, halign: 'center' },
        3: { cellWidth: (PAGE.w - M * 2 - 106) * 0.22, halign: 'center' },
        4: { cellWidth: (PAGE.w - M * 2 - 106) * 0.24, halign: 'center' },
      },
      didParseCell: (data) => {
      ...
      didDrawPage: (data) => {
        if (data.pageNumber > 1) {
          drawTopHeaderOnly(doc, isDuplicate);
        }
      }
    });

    const finalTableY = (doc.lastAutoTable && doc.lastAutoTable.finalY) ? doc.lastAutoTable.finalY : 790;
    ...
    const sigY = Math.max(finalAfterTotalY + 65, bottomPreferredY);

    // ---------- TERMS & CONDITIONS (POSITIONED DIRECTLY ABOVE SIGNATURES) ----------
    const noticeY = sigY - 45;

    setFont(doc, "bold", 9.5, C.dark);
    doc.text("TERMS & CONDITIONS / OPERATIONAL MANDATE", PAGE.w / 2, noticeY, { align: "center" });

    setFont(doc, "bold", 8.5, C.dark);
    doc.text("• Entry Challan creation must be executed within 24 hours of cutting completion.", PAGE.w / 2, noticeY + 12, { align: "center" });
    doc.text("• Design proof approval must be processed and passed within 24 hours of receipt. Strict adherence is required with zero operational delay.", PAGE.w / 2, noticeY + 22, { align: "center" });
  ...
  const handleCreateChallan = async (row, extras = {}) => {
    ...
    let challanNo = '';
    try { challanNo = await reserveChallanNo('CH-PRINT-', 0); }
    catch { challanNo = await reserveChallanNo('CH-PRINT-', 0); }
    ...
    try {
      ...
      let filename = '';
      try {
        const res = await buildA3PortraitChallanPDF(row, challanNo, challanDate, pdfDisplayItems, completeLotFlag, displayChallanNo);
        doc = res.doc;
  ...
  const showBrowserNotification = (notification) => {
    if (browserNotificationsEnabled) {
      new Notification('Printing Challan', {
  ...
  const generateChallanPdfLocal = async (row, entry) => {
    try {
      ...
      const entryDateObj = entry?.date ? new Date(entry.date) : new Date();
      const challanDate = Number.isNaN(entryDateObj.getTime()) ? String(entry?.date || '') : fmtLong(entryDateObj);
      const displayChallanNo = entry?.number || 'CHALLAN';

      const { doc, filename } = await buildA3PortraitChallanPDF(
        row,
        entry?.number || 'CHALLAN',
        challanDate,
        itemsFromEntry,
        entry?.completeLot,
        displayChallanNo,
  ...
  return (
    <div className="mh-ec">
      <div className="shell">
        {/* Header / Hero */}
        <div className="hero">
          <span className="kicker">Printing Suite</span>
          <div className="hero-title">
            <button className="back" onClick={() => window.history.back()} title="Back">← Back</button>
            <h1>Challan Printing Matchboard</h1>
          </div>

          <div className="stats-container">
            <div className="stat-card">
              <div className="stat-lbl">Matched Lots</div>
              <div className="stat-val">{matchedRows.length.toLocaleString()}</div>
            </div>
            <div className="stat-card">
              <div className="stat-lbl">Pending Qty</div>
              <div className="stat-val">{stats.totalPendingQty.toLocaleString('en-IN')} <span className="stat-sub">Pcs</span></div>
            </div>
            <div className="stat-card">
              <div className="stat-lbl">Printing Status</div>
              <div className="stat-val">
                <span style={{ color: 'var(--warn)' }}>{stats.pendingPrintCount}</span> <span style={{ color: 'var(--muted)', fontSize: 13 }}>/</span> <span style={{ color: 'var(--ok)' }}>{stats.completedPrintCount}</span>
              </div>
              <div className="stat-sub">Pending / Completed</div>
            </div>
            <div className="stat-card">
              <div className="stat-lbl">Material Transit</div>
              <div className="stat-val">
                <span style={{ color: 'var(--warn)' }}>{stats.pendingMatCount}</span> <span style={{ color: 'var(--muted)', fontSize: 13 }}>/</span> <span style={{ color: 'var(--ok)' }}>{stats.receivedMatCount}</span>
              </div>
              <div className="stat-sub">Pending / Received</div>
            </div>
          </div>
        </div>
          ...
          <div className="toolbar rel">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <select
                ...
                title="Filter by Material Status"
              >
              ...
              <select
                ...
                title="Filter by Printing Status"
              >
                <option value="ALL">Printing: All</option>
                <option value="COMPLETED">Printing: Completed</option>
                ...
                <table className="minw">
                  ...
                  <tbody>
                    {paginated.map((row, i) => {
                      ...
                      return (
                        <tr key={i}>
                          {/* Render the main data columns */}
                          {renderCols.map((c) => {
                            const isLotCol = c === 'Lot Number' || c === 'Lot No.' || c === 'Lot';
                            return (
                              <td key={c} data-label={c} style={isLotCol ? { fontWeight: '900', color: 'red', fontSize: '15px' } : undefined}>
                                {row[c]}
                              </td>
                            );
                          })}

                          {/* Render the additional columns */}
                          ...
                          {(() => {
                            ...
                            const isComp = status === 'Completed' || embstatus === 'Complete Printing' || isLotMarkedComplete(row);
                            return (
                              <td
                          ...
                          <td data-label="Material Status">
                            {materialStatus ? (
                              <span className={`status-tag status-${materialStatus === 'Material Received' ? 'success' : 'warning'}`} title={materialStatus}>
                          ...
                          <td data-label="Printing Status">
                            {embstatus ? (
                              <span className={`status-tag status-${embstatus === 'Complete Printing' ? 'success' : 'warning'}`} title={embstatus}>
                                <span className="status-dot"></span>
                                {embstatus === 'Complete Printing' ? 'Completed' : 'Pending'}
                              </span>
                            ) : '—'}
                          </td>
                          ...
                          <td data-label="Challan">
                            {(() => {
                              const isCompleted = status === 'Completed' || embstatus === 'Complete Printing' || isLotMarkedComplete(row) || (getPositiveQtyItemsForLot(row).length > 0 && getRemainingItemsForLot(row).length === 0);
                              return (
                                <button
                                  className={isCompleted ? "btn btn-ghost" : "btn btn-primary"}
                                  style={{ height: 32, padding: '0 10px', borderRadius: 8, ...(isCompleted ? { opacity: 0.6, cursor: 'not-allowed' } : {}) }}
                            ...
                            <div className="hc-actions" style={{ textAlign: 'right', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                              ...
                              >
                                {markingEmbId === `${String(historyRow['Lot Number'] ?? historyRow['Lot'] ?? '')}__${e?.number || i}__emb`
                                  ? (<><span className="spin" aria-hidden="true" />Updating…</>)
                                  : (e?.embCompleted ? 'Unmark Printing' : 'Mark Printing Completed')}
                              </button>
            ...
            <div className="modal-b" style={{ overflow: 'auto', flex: 1, maxHeight: 'calc(90vh - 120px)' }}>
              ...
              <div style={{ overflow: 'auto', maxHeight: '300px' }}>
                ...
                >
                  <thead>
                    <tr>
                      <th style={{ width: 52, textAlign: 'center', position: 'sticky', top: 0, background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', color: '#ffffff' }}>#</th>
                      <th style={{ position: 'sticky', top: 0, background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', color: '#ffffff' }}>Shade</th>
                      <th style={{ width: 140, position: 'sticky', top: 0, background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', color: '#ffffff' }}>Quantity</th>
                      <th style={{ width: 60, position: 'sticky', top: 0, background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', color: '#ffffff' }}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((it, idx) => (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const cleanItems = items
                    .map((it) => ({ shade: String(it.shade || '').trim(), qty: String(it.qty || '').trim(), tableNo: String(it.tableNo || '').trim() }))
        ...
        >
          ...
          >
              ...
              {selectedJobOrder ? (
                <div className="job-order-grid">
                  ...
                  {[
                    ...
                    { key: 'Printing Details', label: 'Printing Details' },
                    ...
                    if (key === 'Image URL' && value) {
                      return (
                        <div key={key} className="job-order-field">
                          <div className="job-order-label">{label}</div>
                          <div className="job-order-value">
                            <button
                              className="btn btn-primary"
                              onClick={() => {
                                // Open image in new tab or show in iframe
            ...
            <div className="job-order-actions">
              <button className="btn btn-primary" onClick={closeJobOrderDialog}>
                Close
              </button>
            </div>
          </div>
...
export default PrintingChallan;
