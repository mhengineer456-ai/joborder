...
const fetchJobOrderValues = async () => {
  try {
    ...
    const jobOrderRange = `${jobOrderSheetName}!A2:Z`;
...
const autoFillRangeDialog = async () => {
    ...
    try {
      ...
      try {
        ...
        if (loadRes && loadRes.ok) {
          ...
          if (loadRes.rolls) setRolls(loadRes.rolls);
          if (loadRes.kgs) setKgs(loadRes.kgs);
          if (loadRes.kapdaLayerWT) setKapdaLayerWT(loadRes.kapdaLayerWT);
          if (loadRes.layerPcs) setLayerPcs(loadRes.layerPcs);
          if (loadRes.layerInch) setLayerInch(loadRes.layerInch);
          if (loadRes.dia) setDia(loadRes.dia);
          if (loadRes.cuttingWeight) setCuttingWeight(loadRes.cuttingWeight);
          if (loadRes.kapdaWapsi) setKapdaWapsi(loadRes.kapdaWapsi);
          
          // Load new columns if they exist
          if (loadRes.proposedWeightPerPcs) setProposedWeightPerPcs(loadRes.proposedWeightPerPcs);
          if (loadRes.netWeight) setNetWeight(loadRes.netWeight);
          if (loadRes.actualWeightPerPcs) setActualWeightPerPcs(loadRes.actualWeightPerPcs);
          if (loadRes.diff) setDiff(loadRes.diff);
          
          // Load weight per piece columns if they exist
          if (loadRes.grossWeightPerPcs) setGrossWeightPerPcs(loadRes.grossWeightPerPcs);
          if (loadRes.netWeightPerPcs) setNetWeightPerPcs(loadRes.netWeightPerPcs);
      ...
      for (const sh of finalShades) {
        ...
        initRolls[sh] = String(rolls[sh] ?? "");
        initKgs[sh] = String(kgs[sh] ?? "");
        initKapdaLayerWT[sh] = String(kapdaLayerWT[sh] ?? "");
        initLayerPcs[sh] = String(layerPcs[sh] ?? "");
        initLayerInch[sh] = String(layerInch[sh] ?? "");
        initDia[sh] = String(dia[sh] ?? "");
        initCuttingWeight[sh] = String(cuttingWeight[sh] ?? "");
        initKapdaWapsi[sh] = String(kapdaWapsi[sh] ?? "");
        
        // Calculate proposed weight per pcs on initialization
        const layerWT = parseDecimal(kapdaLayerWT[sh] || 0);
        const layerPcsVal = parseDecimal(layerPcs[sh] || 0);
        ...
        initProposedWeightPerPcs[sh] = calculatedProposedWeight || String(proposedWeightPerPcs[sh] ?? "");
        initNetWeight[sh] = calculatedNetWeight || String(netWeight[sh] ?? "");
        initGrossWeightPerPcs[sh] = calculatedGrossWeightPerPcs || String(grossWeightPerPcs[sh] ?? "");
        initNetWeightPerPcs[sh] = calculatedNetWeightPerPcs || String(netWeightPerPcs[sh] ?? "");
        initActualWeightPerPcs[sh] = calculatedActualWeightPerPcs || String(actualWeightPerPcs[sh] ?? "");
        initDiff[sh] = calculatedDiff || String(diff[sh] ?? "");
      }
  ...
  }, [apiKey, headerRange, rowRangePrefix, sheetName, spreadsheetId, lot, headers.length, num]);
  // Handle save range
...
.diff-exceeds-standard::before {
  ...
  font-weight: 800;
    ...
    .summary-card:hover .summary-card-progress-bar {
        width: 100%;
    }

    /* Card color variants with improved gradients */
    .card-primary { 
    ...
    .card-primary .summary-card-progress-bar { background: linear-gradient(90deg, var(--brand), #60A5FA); }
    ...
    .card-success .summary-card-progress-bar { background: linear-gradient(90deg, var(--accent), #34D399); }
    ...
    .card-warning .summary-card-progress-bar { background: linear-gradient(90deg, var(--warning), #FBBF24); }

    .card-danger { 
        border-left: 4px solid var(--danger);
        background: linear-gradient(135deg, #FFFFFF 0%, #FEF0F0 100%);
    }
    .card-danger .summary-card-icon { 
        background: linear-gradient(135deg, #FEF2F2 0%, #FEE2E2 100%); 
        color: var(--danger); 
        box-shadow: 0 4px 12px rgba(239, 68, 68, 0.15);
    }
    .card-danger .summary-card-progress-bar { background: linear-gradient(90deg, var(--danger), #F87171); }
    ...
    .card-info .summary-card-progress-bar { background: linear-gradient(90deg, #7C3AED, #A78BFA); }
    ...
    .summary-card-trend {
        ...
        thead th{
          ...
          color:white; font-size:12px; text-transform:uppercase; letter-spacing:.06em; font-weight:800;
          padding:10px 8px; border-bottom:1px solid rgba(0,0,0,.06); z-index:2;
        }
        ...
        .wapsi-input:focus,
        .cutting-input:focus,
        .proposed-weight-input:focus,
        .net-weight-input:focus,
        .actual-weight-input:focus,
        .diff-input:focus,
        .gross-weight-pcs-input:focus,
        .net-weight-pcs-input:focus {
          box-shadow: var(--ring);
        ...
        .actual-weight-input, .diff-input {
        ...
        <div className="modern-header">
          <div className="header-content">
            ...
            <div className="search-section" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              {/* Add this button in the search-section div, after the Refresh button */}
...
<button
  ...
  className="btn"
  title="Define measurement ranges"
>
  📏 Define Range
</button>
        ...
        <div className="main-content" aria-live="polite">
          ...
          {shades.length > 0 && (
            <div className="summary-cards">
              ...
              {/* New Total card */}
            {/* New Total card - Updated with range validation */}
...
{/* Add these two cards after the Standard card */}
                  ...
                  <table className="data-table" role="grid" aria-rowcount={shades.length}>
                    <thead>
                      <tr>
                        ...
                        <th>Diff</th>
                        {/* <th>Gross Weight/pcs</th>
                        <th>Net Weight/pcs</th> */}
                      </tr>
                    </thead>
                    ...
                    <tbody>
                      ...
                      {(useVirtual ? shades.slice(startIdx, endIdx) : shades).map((shade, idx) => {
                        ...
                        // Calculate gross weight per pcs automatically (Kgs / Total Pcs)
                        const autoCalculatedGrossWeightPerPcs = totalPcs > 0 ? 
                          (kgsVal / totalPcs).toFixed(3) : "";
                        
                        // Calculate net weight per pcs automatically (Net Weight / Total Pcs)
                        const autoCalculatedNetWeightPerPcs = totalPcs > 0 ? 
                          (parseDecimal(autoCalculatedNetWeight) / totalPcs).toFixed(3) : "";
...
<td>
  ...
  />
    ...
    title={entry.includeInTotal ? "Included in total" : "Excluded from total"}
